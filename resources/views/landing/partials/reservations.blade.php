        ...
        <form action="{{ route('reservations.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <!-- 🟢 Select dinámico con corrección de contraste para los textos -->
                    <select name="area" class="w-full bg-stone-950 border border-stone-800 rounded-xl px-4 py-3 text-white text-sm focus:border-amber-500 focus:ring-0 appearance-none" required>
                        <option value="" class="bg-stone-900 text-stone-400">Selecciona el área...</option>
                        
                        @if(!empty($areas))
                            @foreach($areas as $key => $value)
                                <option value="{{ $key }}" class="bg-stone-900 text-white">
                                    {{ $value }}
                                </option>
                            @endforeach
                        @else
                            <!-- Fallback en caso de que esté vacío el seeder -->
                            <option value="GENERAL" class="bg-stone-900 text-white">Zona General / Tablado</option>
                            <option value="VIP" class="bg-stone-900 text-white">Palcos / Zona VIP</option>
                        @endif
                    </select>
                </div>
            </div>
